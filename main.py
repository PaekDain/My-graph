...
st.markdown("일별 박스오피스 데이터를 바탕으로 시간의 흐름에 따른 영화 관객 수 및 각종 지표 변화를 탐색합니다.")
st.markdown("---")
...
if len(selected_date_range) == 2:
    ...
    df = raw_df[(raw_df['날짜'].dt.date >= start_date) & (raw_df['날짜'].dt.date <= end_date)].copy()
else:
    df = raw_df.copy()
...
if not movie_df.empty:
    ...
    st.warning(f"선택한 기간 내에 [{selected_movie}]의 상영 데이터가 없습니다.")
...
# -----------------------------------------------------------------------------
st.header("📌 구역 3: 날짜별 박스오피스 총 관객 수 추이 및 특정 영화 비교")
...
if selected_movies_q3:
    ...
    chart_title = f"선택한 {len(selected_movies_q3)}개 영화의 일별 관객 수 총합"
else:
    filtered_q3_df = df
    chart_title = "일별 박스오피스 전체 영화 관객 수 총합"

daily_total = filtered_q3_df.groupby('날짜')['일관객'].sum().reset_index().sort_values('날짜')
...
selected_movie_q6 = st.selectbox(
    "조회할 영화를 선택하세요 (전체 개봉 영화):",
    options=all_movies_sorted,
    index=0,
    key="select_q6"
)

# 선택한 영화 전체 데이터 추출 (사이드바 날짜 필터 영향 없는 원본 기준)
...
if not q6_movie_df.empty:
    ...
    m_col2.metric("👥 총 관객 수 (기간 합계 / 최고 누적)", f"{total_period_audience:,} 명", f"최종 {max_accum_audience:,.0f} 명")
    m_col3.metric("📅 첫 상영 날짜", first_date)
    m_col4.metric("🏁 마지막 상영 날짜", last_date)

    st.subheader(f"📊 [{selected_movie_q6}] 상영 정보 상세")
    st.dataframe(
        q6_movie_df[['날짜', '순위', '일관객', '누적관객', '스크린수', '상영횟수']].assign(
            날짜=q6_movie_df['날짜'].dt.strftime('%Y-%m-%d')
        ),
...
user_input_6 = st.text_area("💡 이 구역으로 알 수 있는 것 (직접 입력):", value=st.session_state.note_6, key="input_6", placeholder="영화 상세 정보 분석 인사이트를 기록하세요...")
